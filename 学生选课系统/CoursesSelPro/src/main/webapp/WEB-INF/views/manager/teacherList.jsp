...
<rapid:override name="content">
    ...
    %>
    <div>
        <input type="button" onclick="insert()" value="添加教师" class="layui-btn layui-btn-lg layui-btn-warm" style="margin-top:15px;">
    </div>
    <script>
        function insert() {
            window.location.href="<%=basePath%>manager/insertTeacher";
        }
    </script>
    ...
    <table class="layui-table" style="margin-top:15px;">
        ...
        <c:forEach items="${paging.dataList}" var="teacher">
            <tr>
                ...
                <td>
                    <button class="layui-btn" onclick="delete_fun(${teacher.teaId})"  >删除
                    </button>     
                </td>
            </tr>
        </c:forEach>
    ...
    <script>
        function search1() {
            var tid = document.getElementById("searchId").value;
            window.location.href = "<%=basePath%>manager/searchByTid?tId=" + tid;
        }
        function search2() {
            var tName = document.getElementById("searchName").value;
            window.location.href = "<%=basePath%>manager/searchByTname?tName=" + tName;
        }

        function goPage(page) {
            window.location.href = "<%=basePath%>manager/teacherList?page=" + page;
        }
        ...
        function delete_fun(teaId) {
            var r = confirm("确认删除吗？")
            if (r == true) {
                window.location.href = "<%=basePath%>manager/deleteTeacher?teaid=" + teaId;
            }
            else {
                return;
            }
        ...
        $(function () {
            $("#tea").click(function () {
                ...
                if(teaid===0){
                    ...
                    window.location.href="<%=basePath%>student/searchListByTeaId?teaid="+teaid+"&stuid="+stuid;
                }                                                 
            });
            $("#ins").click(function () {
